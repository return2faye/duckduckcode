...
| 模板补强后 | 5/5 | 41/41（100%） | 所有精确记录保留，无工具注入提升，标签完整 |
...
- 模板要求完整复制 `key=value` 记录，不得重命名 key 或释义 value。
- 模板要求按时间应用状态迁移，后续成功修改或验证覆盖较早的 pending 状态。

修复后的 5 次请求总 token usage 为 4,680。该数字来自 provider 返回的总 token，不区分输入、输出或 cache hit。

本轮没有用 DeepSeek 重跑 4 个端到端 context bench：此前 `o4-mini` 基线为 4/4；DeepSeek SWE pilot 已显示 thinking tool loop 成本异常，先修复该 provider 生命周期问题再运行端到端套件，避免把工具协议失败混入摘要质量分数。
...
- 当前每个场景只采样一次，不提供随机性置信区间；需要比较模型稳定性时再运行至少 3 次并报告最差值。
