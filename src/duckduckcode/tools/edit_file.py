...
def create_edit_file_tool(working_directory: Path | None = None) -> Tool:
    ...
    return create_tool(
        "EditFile",
        "Use EditFile for targeted edits to an existing UTF-8 text file after using ReadFile first. Pass an absolute file path inside the working directory or private temporary directory; relative paths are accepted only for compatibility. Provide old_string with enough exact context to match once, then new_string; the tool returns numbered context near the change.",
        EDIT_FILE_PARAMS,
