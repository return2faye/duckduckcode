...
def create_read_file_tool(working_directory: Path | None = None) -> Tool:
    ...
    return create_tool(
        "ReadFile",
        "Use ReadFile to inspect an existing UTF-8 text file before proposing edits or before EditFile. Pass an absolute file path inside the working directory or private temporary directory; relative paths are accepted only for compatibility. Use offset and limit to read large files in chunks. Prefer Glob to find file paths and Grep to search contents.",
        READ_FILE_PARAMS,
